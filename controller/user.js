...
router.post('/login',async function(req,res,next){
	...
	await new Promise((resolve,reject)=>{
		hash({ password: user.password }, function (err, pwd, salt, hash1) {
  ...
  hash({ password: pwd, salt: user.salt }, function (err, pwd, salt, hash) {
			console.log('2222');
...
router.post('/register',function(req,res,next){
	...
	userDao.addAUserWithNamAndPwd(name,pwd).then(()=>{
		res.sendFile(path.join(__dirname, 'lgn_rgt.html'));
//		
	});
